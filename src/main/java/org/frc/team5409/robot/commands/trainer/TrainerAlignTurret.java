...
public final class TrainerAlignTurret extends CommandBase {
    ...
    private enum CommandState {
        kSearching, kSweeping, kAligning, kEnd
    }
    ...
    public TrainerAlignTurret(TrainerDashboard dashboard, TrainingContext context, ShooterFlywheel sys_flywheel, ShooterTurret sys_rotation, Limelight sys_limelight, Indexer sys_indexer) {
    ...
    private void internal_switchState(CommandState state) {
        ...
        } else if (state == CommandState.kEnd) {
            m_shooter_flywheel.setSafety(true);
            m_shooter_flywheel.disable();
    ...
    private void internal_operateAligning(double time) {
        if (m_limelight.hasTarget() && m_limelight.getTargetType() == Limelight.TargetType.kOuterPort) {
            ...
            m_training_context.setDistance(m_distance);
