...
public final class RunTurretFlywheel extends CommandBase {
    ...
    private       double         m_target, m_distance;

    private final Range          m_distance_range;
    private final SimpleEquation m_distance_rpm_curve;
    ...
    public void execute() {
        if (m_limelight.hasTarget() && m_limelight.getTargetType() == Limelight.TargetType.kOuterPort) {
            ...
            m_distance = m_distance_range.clamp(m_height / Math.tan(target.y + Constants.Vision.vision_limelight_pitch));
            m_target = m_distance_rpm_curve.calculate(m_distance);
