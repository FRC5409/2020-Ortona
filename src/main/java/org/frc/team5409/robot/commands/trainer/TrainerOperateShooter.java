 ...
 * @author Keith Davies
 */
public final class TrainerOperateShooter extends CommandBase {
    /**
    ...
    public TrainerOperateShooter(TrainingContext context, ShooterFlywheel sys_flywheel, ShooterTurret sys_rotation, Limelight sys_limelight, Indexer sys_indexer) {
    ...
    private void internal_operateShooting(double time) {
        if (m_limelight.hasTarget() && m_limelight.getTargetType() == Limelight.TargetType.kOuterPort) {
            ...
            m_shooter_flywheel.setVelocity(m_training_context.getSetpoint().getTarget());
            m_shooter_turret.setRotation(m_shooter_turret.getRotation()+target.x);
            SmartDashboard.putNumber("Aligning offset", target.x);
        }
